...
static void appl_init(void) {
  ...
  queue_example_init();//-- init common application objects
  //-- create all the rest application tasks:


  {
    task_consumer_create();//-- create the consumer task {{{
    //-- wait until consumer task initialized
    SYSRETVAL_CHECK(tn_eventgrp_wait(
        queue_example_eventgrp_get(), QUE_EXAMPLE_FLAG__TASK_CONSUMER_INIT,
        TN_EVENTGRP_WMODE_AND, TN_NULL, TN_WAIT_INFINITE));
  }
  // }}}
}
...
static void task_producer_body(void *par) {
  ...
  SYSRETVAL_CHECK(tn_eventgrp_modify(queue_example_eventgrp_get(),
                                     TN_EVENTGRP_OP_SET,
  ...
  for (;;) {
    ...
    for (i = 0; i < 3 /*pins count*/; i++) {
      ...
      if (!task_consumer_msg_send(TASK_CONS_CMD__PIN_TOGGLE, pin_num)) {
...
void task_producer_create(void) {
  SYSRETVAL_CHECK(tn_task_create(&task_producer, task_producer_body,
                                 TASK_PRODUCER_PRIORITY, task_producer_stack,
                                 TASK_PRODUCER_STACK_SIZE, TN_NULL,
