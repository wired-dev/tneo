...
static void task_consumer_body(void *par) {
  ...
  SYSRETVAL_CHECK(tn_fmem_create(
      &cons_fmem, cons_fmem_buf,
      TN_MAKE_ALIG_SIZE(sizeof(struct TaskConsumerMsg)), CONS_QUE_BUF_SIZE));
  //-- create queue
  SYSRETVAL_CHECK(
...
void task_consumer_create(void) {
  SYSRETVAL_CHECK(tn_task_create(&task_consumer, task_consumer_body,
                                 TASK_CONSUMER_PRIORITY, task_consumer_stack,
                                 TASK_CONSUMER_STACK_SIZE, TN_NULL,
...
int task_consumer_msg_send(enum E_TaskConsCmd cmd, enum E_TaskConsPin pin_num) {
  ...
  tn_rc =
      SYSRETVAL_CHECK_TO(tn_fmem_get(&cons_fmem, (void *)&p_msg, WAIT_TIMEOUT));
  if (tn_rc == TN_RC_OK) {
